...
class TokenSquashCodecTests(unittest.TestCase):
    ...
    def test_product_manifest_lists_contract_surface(self) -> None:
        ...
        self.assertIn("tokensquash.turns.scorecard.pack.v1", schemas)
        ...
        self.assertTrue(any("release-assets" in command and "v0.2.1" in command for command in readiness_commands))
        ...
        self.assertIn("docs/release-candidate.md", governance_paths)
        ...
        self.assertIn("docs/release-notes-v0.2.1.md", governance_paths)
        ...
        self.assertIn("docs/sidecar-ollama.md", governance_paths)
        ...
        self.assertIn(".github/ISSUE_TEMPLATE/feature_request.yml", governance_paths)
        self.assertIn(".github/ISSUE_TEMPLATE/commercial_licensing.yml", governance_paths)
        self.assertIn(".github/ISSUE_TEMPLATE/private_data_security.yml", governance_paths)
    ...
    def test_product_readiness_writes_evidence_pack(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            out_dir = Path(tmp) / "readiness"

            report = run_product_readiness(out_dir=out_dir, skip_tests=True)

            self.assertEqual(report["schema_version"], "tokensquash.readiness.v1")
            self.assertEqual(report["status"], "pass")
    ...
    def test_product_readiness_cli_json(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            out_dir = Path(tmp) / "readiness"
            stdout = StringIO()

            with redirect_stdout(stdout):
                code = cli_main(["readiness", "--skip-tests", "--out-dir", str(out_dir), "--json"])

            payload = json.loads(stdout.getvalue())
            self.assertEqual(code, 0)
            self.assertEqual(payload["schema_version"], "tokensquash.readiness.v1")
            self.assertEqual(payload["status"], "pass")
    ...
    def test_release_candidate_writes_evidence_pack(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
    ...
    def test_release_assets_stages_public_assets(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue(report["summary"]["verification_doc_updated"])
    ...
    def test_suggest_sidecar_review_groups_flags(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
                ...
                json.dumps(
                    {
                        ...
                        "rows": [
                            ...
                            },
                            {
                                ...
                                "risk_level": "low",
                                "risk_score": 15,
                                "flags": ["missing_command_or_verification"],
                                "saved_tokens": 80,
                                "saved_pct": 30.0,
                                "original_preview": "Done. Ran pytest.",
                                "decoded_preview": "Done.",
                            },
                        ],
                    }
                ),
            ...
            report = suggest_sidecar_review(review)

        self.assertEqual(report["schema_version"], "tokensquash.sidecar.suggestions.v1")
        self.assertEqual(report["status"], "pass")
        self.assertEqual(report["summary"]["suggestion_count"], 2)
        self.assertEqual(report["flag_counts"]["high_savings_short_decoded"], 1)
        self.assertEqual(report["flag_counts"]["missing_command_or_verification"], 1)
        self.assertEqual(report["suggestions"][0]["flag"], "high_savings_short_decoded")
        self.assertIn("Preserve more meaning", report["suggestions"][0]["title"])

    def test_sidecar_suggestions_cli_writes_outputs(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            review = Path(tmp) / "review.json"
            out_dir = Path(tmp) / "suggestions"
            review.write_text(
                json.dumps(
                    {
                        "schema_version": "tokensquash.sidecar.review.v1",
                        ...
                        "rows": [
                            {
                                ...
                                "saved_tokens": 50,
                                "saved_pct": 25.0,
                                "original_preview": "Done. Changed src/app.py.",
                                "decoded_preview": "Done.",
                            }
                        ],
                    }
            ...
            with redirect_stdout(stdout):
                code = cli_main(["sidecar", "suggestions", str(review), "--out-dir", str(out_dir), "--json"])

            payload = json.loads(stdout.getvalue())
            self.assertEqual(code, 0)
            self.assertEqual(payload["schema_version"], "tokensquash.sidecar.suggestions.v1")
            self.assertEqual(payload["suggestions"][0]["flag"], "missing_files")
    ...
    def test_turns_scorecard_pack_cli_json_writes_artifacts(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "turns.jsonl"
            ...
            payload = json.loads(stdout.getvalue())
            self.assertEqual(code, 0)
            self.assertEqual(payload["schema_version"], "tokensquash.turns.scorecard.pack.v1")
            self.assertEqual(payload["summary"]["history_included"], False)
            self.assertTrue((out_dir / "scorecard.json").exists())
            self.assertTrue((out_dir / "scorecard.md").exists())
            self.assertFalse((out_dir / "history.json").exists())
            scorecard = json.loads((out_dir / "scorecard.json").read_text(encoding="utf-8"))
            self.assertEqual(scorecard["schema_version"], "tokensquash.turns.scorecard.v1")
            self.assertIn("TokenSquash Turn Scorecard", (out_dir / "scorecard.md").read_text(encoding="utf-8"))

    def test_turns_scorecard_pack_writes_history(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "turns.jsonl"
            previous = Path(tmp) / "previous-scorecard.json"
            out_dir = Path(tmp) / "pack"
            path.write_text(
                '{"id":"t1","prompt":"review src/auth.py","reply":"Done. I reviewed src/auth.py and ran tests."}\n',
                encoding="utf-8",
            ...
            report = write_turn_scorecard_pack(
                ...
                counter="chars",
                auto_sidecar=False,
                history_scorecards=[previous],
            )

            self.assertEqual(report["schema_version"], "tokensquash.turns.scorecard.pack.v1")
            self.assertEqual(report["summary"]["history_included"], True)
            self.assertEqual(report["summary"]["history_input_count"], 1)
            self.assertTrue((out_dir / "scorecard.json").exists())
            self.assertTrue((out_dir / "scorecard.md").exists())
            self.assertTrue((out_dir / "history.json").exists())
            self.assertTrue((out_dir / "history.md").exists())
            history = json.loads((out_dir / "history.json").read_text(encoding="utf-8"))
            self.assertEqual(history["schema_version"], "tokensquash.turns.scorecard.history.v1")
            self.assertEqual(history["summary"]["scorecard_count"], 2)
            self.assertIn("TokenSquash Turn Scorecard History", (out_dir / "history.md").read_text(encoding="utf-8"))
    ...
    def test_turns_scorecard_history_reports_trend(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            first = Path(tmp) / "scorecard-1.json"
            second = Path(tmp) / "scorecard-2.json"
            third = Path(tmp) / "scorecard-3.json"
                    ...
                    json.dumps(
                        {
                            ...
                            "summary": {
                                "turn_count": turns,
                                "milestone": milestone,
                                "original_tokens": 1000 + turns,
                                "squashed_tokens": 900 + turns,
                                "saved_tokens": saved_tokens,
                                "saved_pct": saved_pct,
                                "prompt_saved_pct": saved_pct - 1.0,
                                "reply_saved_pct": saved_pct + 1.0,
                                "privacy_finding_count": 0,
                                "pass_through_rows": 0,
                                "raw_wire_loss_turns": 0,
                                "selected_path_prefix_count": 1,
                                "selected_field_value_count": 1,
                                "alias_saved_tokens_delta": 10,
                                "break_even_corpora": 3,
                                "sidecar_status": "pass" if status == "pass" else "missing",
                                "sidecar_pass_count": 2 if status == "pass" else 0,
                                "sidecar_watch_count": 0,
                                "sidecar_fail_count": 0,
                            },
                        }
            ...
            write_scorecard(first, status="pass", turns=10, milestone="smoke", saved_pct=4.0, saved_tokens=40)
            write_scorecard(second, status="pass", turns=25, milestone="early_pattern", saved_pct=6.0, saved_tokens=75)
            write_scorecard(third, status="watch", turns=30, milestone="early_pattern", saved_pct=5.5, saved_tokens=82)

            report = build_turn_scorecard_history([first, second, third])

            self.assertEqual(report["schema_version"], "tokensquash.turns.scorecard.history.v1")
            self.assertEqual(report["status"], "watch")
            self.assertEqual(report["summary"]["scorecard_count"], 3)
            self.assertEqual(report["summary"]["turn_count_delta"], 20)
            self.assertEqual(report["summary"]["saved_pct_delta"], 1.5)
            self.assertEqual(report["summary"]["saved_tokens_delta"], 42)
            self.assertEqual(report["summary"]["pass_step_count"], 1)
            self.assertEqual(report["summary"]["watch_step_count"], 1)
            self.assertEqual(report["summary"]["fail_step_count"], 0)
            self.assertEqual(report["summary"]["best_scorecard_path"], str(second))
            self.assertEqual(report["summary"]["saved_pct_drop_from_best"], -0.5)
            self.assertEqual(len(report["scorecards"]), 3)
            self.assertEqual(len(report["steps"]), 2)
            self.assertTrue(any("latest scorecard is watch" in warning for warning in report["warnings"]))

    def test_turns_scorecard_history_cli_markdown_accepts_directories(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            first_dir = Path(tmp) / "first"
            second_dir = Path(tmp) / "second"
            first_dir.mkdir()
            second_dir.mkdir()
            out = Path(tmp) / "scorecard-history.md"
            first_payload = {
                "schema_version": "tokensquash.turns.scorecard.v1",
                "status": "pass",
                "summary": {
            ...
            (first_dir / "scorecard.json").write_text(json.dumps(first_payload), encoding="utf-8")
            (second_dir / "scorecard.json").write_text(json.dumps(second_payload), encoding="utf-8")
            stdout = StringIO()

            with redirect_stdout(stdout):
                code = cli_main(["turns", "scorecard-history", str(first_dir), str(second_dir), "--out", str(out)])

            output = stdout.getvalue()
            self.assertEqual(code, 0)
            self.assertIn("# TokenSquash Turn Scorecard History", output)
            self.assertIn("## Scorecard Timeline", output)
    ...
    def test_turns_release_check_runs_certification_history_and_doctor(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(report["schema_version"], "tokensquash.turns.release_check.v1")
            self.assertEqual(report["status"], "pass")
            self.assertEqual(report["summary"]["certification_status"], "pass")
            self.assertEqual(report["summary"]["doctor_status"], "pass")
            self.assertEqual(report["summary"]["history_status"], "same")
            self.assertEqual(report["summary"]["failed_required_count"], 0)
    ...
    def test_turns_release_check_cli_json_writes_warn_pack_without_history(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            payload = json.loads(stdout.getvalue())
            self.assertEqual(code, 0)
            self.assertEqual(payload["schema_version"], "tokensquash.turns.release_check.v1")
            self.assertEqual(payload["status"], "warn")
            self.assertEqual(payload["summary"]["history_status"], "skip")
            self.assertEqual(payload["summary"]["certification_status"], "pass")
            self.assertEqual(payload["summary"]["doctor_status"], "pass")
    ...
    def test_turns_verify_release_pack_passes_for_complete_evidence(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("certification_evaluation_validation", names)
            self.assertIn("certification_evaluation_alias_table", names)
    ...
    def test_turns_verify_release_cli_json_fails_missing_nested_evidence(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (release_dir / "certification" / "evaluation" / "aliases.json").unlink()
            (release_dir / "doctor-strict" / "gate.json").unlink()
            (release_dir / "doctor-strict" / "evaluation" / "bench.json").unlink()
            stdout = StringIO()
            ...
            self.assertIn("certification_evaluation_alias_table", failed_names)
            self.assertIn("doctor_strict_gate", failed_names)
            self.assertIn("doctor_strict_evaluation_bench", failed_names)
